...
router.get('/pdf/:month?', auth, async (req, res) => {
  try {
    ...
    // Summary - FIXED type logic (assuming type: 1=expense, -1=income)
    let yPosition = 150;
    ...
    transactions.slice(0, 20).forEach((tx) => {
      const typeIcon = tx.type === 1 ? '💸 Expense' : '💰 Income';  // Fixed
      doc.fontSize(11)
...
// CSV EXPORT - No changes needed
router.get('/csv/:month?', auth, async (req, res) => {
  // ... your existing CSV code is perfect
});
